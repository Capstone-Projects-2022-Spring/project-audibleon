...
class SignupForm(FlaskForm):
    ...
    email = StringField(label='Email', validators=[InputRequired(), Email(message='Enter a Valid Email')], render_kw={'placeholder': 'Email Address'})
    username = StringField(label='Username', validators=[InputRequired(), Length(min=4, max=20)], render_kw={'placeholder': 'Username'})
    password = PasswordField(label='Password', validators=[InputRequired(), Length(min=6, max=20, message='Password Must be Between 6 and 20 Characters')], render_kw={'placeholder': 'Password'})
    confirm_password = PasswordField(label='Confirm Password', validators=[InputRequired(), EqualTo('password', message='Passwords Must Match.')], render_kw={'placeholder': 'Confirm Password'})
